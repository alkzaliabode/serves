{{-- resources/views/daily_statuses/partials/employee_leave_item.blade.php --}}
{{-- يستخدم هذا الملف الجزئي للإجازات التي تعتمد على الموظف فقط (مثل الدورية، السنوية، استراحة خفر، بدون راتب، وفاة) --}}

<div class="row mb-2 align-items-center border p-2 rounded bg-light repeater-item">
    <div class="col-md-5">
        <div class="form-group mb-0">
            {{-- تأكد من أن $type و $index يتم تمريرهما إلى هذا الملف الجزئي --}}
            <label for="{{ $type ?? 'default_type' }}_{{ $index }}_employee_id">الموظف:</label>
            <select name="{{ $type }}[{{ $index }}][employee_id]"
                    id="{{ $type ?? 'default_type' }}_{{ $index }}_employee_id"
